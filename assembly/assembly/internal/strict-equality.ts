...
function compareStrictEqualityHookedClass<T>(
	...
	if (
		isDefined(changetype<nonnull<T>>(leftReference).__asHarnessStrictEquals)
	) {
		// @ts-ignore The hook may be supplied either by the transform or explicitly
		// by consumer-defined types.
...
export function compareStrictEqualityString(
	...
	return left == right ? StrictEqualityResult.Match : StrictEqualityResult.Fail;
}
...
export function compareStrictEqualityArrayBufferView(
	...
	if (
		getStrictEqualityRuntimeTypeId(leftReference) !=
		getStrictEqualityRuntimeTypeId(rightReference)
	) {
		return StrictEqualityResult.Fail;
	}
...
export function compareStrictEqualityArrayLike<T>(
	left: T,
	right: T,
): StrictEqualityResult {
	const leftReference = changetype<usize>(left);
...
function pushActiveStrictEqualityReferencePair(
	left: usize,
	right: usize,
): void {
...
export function __asHarnessStrictEqualsArrayBufferMember(
	...
	return (
		compareStrictEqualityArrayBuffer(left, right) != StrictEqualityResult.Fail
	);
}
...
export function __asHarnessStrictEqualsManagedClassMember<T>(
	...
	return (
		compareStrictEqualityManagedClass(left, right) != StrictEqualityResult.Fail
	);
}
