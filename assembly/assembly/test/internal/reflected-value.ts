...
function testArrayLikeReflectedValues(): void {
	...
	reflected =
		createReflectedValue<ReflectedValueUnmanagedArrayLike>(unmanagedArrayLike);
	assert(reflected.kind == ReflectedValueKind.ArrayLike);
...
function testMapReflectedValues(): void {
	...
	assert(reflected.kind == ReflectedValueKind.Map);
	assert(reflected.entries !== null);
	const nestedEntries = changetype<Array<ReflectedValueEntry>>(
		reflected.entries,
	);
	...
	assert(reflected.kind == ReflectedValueKind.Map);
	assert(reflected.entries !== null);
	const recursiveEntries = changetype<Array<ReflectedValueEntry>>(
		reflected.entries,
	);
	assert(recursiveEntries.length == 1);
	assert(
		recursiveEntries[0].value.kind == ReflectedValueKind.CircularReference,
	);
}
...
function testManagedClassReflectedValues(): void {
	...
	const safeLeafPairs =
		changetype<Array<ReflectedValueKeyValuePair>>(leafPairs);
	...
	const safeRootPairs =
		changetype<Array<ReflectedValueKeyValuePair>>(rootPairs);
