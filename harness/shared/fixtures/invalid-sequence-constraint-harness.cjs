...
const NODE_METADATA_BY_INDEX = new Map([
	["0", { nodeId: 1, parentNodeId: 0, declarationOrder: 0, sequenceMode: 9 }],
...
class FakeHarness {
	...
	discover(nodeIndex) {
		switch (Array.isArray(nodeIndex) ? nodeIndex.join(".") : "<invalid>") {
			case "":
				this.#emitNode([0], NODE_KIND_SUITE, "invalid sequence suite");
				this.#emitNode([1], NODE_KIND_SUITE, "plain suite");
				return true;
			case "0":
				this.#emitNode([0, 0], NODE_KIND_TEST, "blocked child a");
				this.#emitNode([0, 1], NODE_KIND_TEST, "blocked child b");
				return true;
			case "1":
