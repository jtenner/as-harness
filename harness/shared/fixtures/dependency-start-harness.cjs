...
const NODE_METADATA_BY_INDEX = new Map([
	...
	["0.0", { nodeId: 10, parentNodeId: 1, declarationOrder: 3 }],
	[
		"1.0",
		{
			nodeId: 11,
	...
	["2.0", { nodeId: 12, parentNodeId: 3, declarationOrder: 5 }],
]);
...
class FakeHarness {
	...
	discover(nodeIndex) {
		switch (Array.isArray(nodeIndex) ? nodeIndex.join(".") : "<invalid>") {
			case "":
				this.#emitNode([0], NODE_KIND_SUITE, "prereq branch");
				this.#emitNode([1], NODE_KIND_SUITE, "missing dependency branch");
				this.#emitNode([2], NODE_KIND_SUITE, "plain branch");
				return true;
			case "0":
				this.#emitNode([0, 0], NODE_KIND_TEST, "prereq");
				return true;
			case "1":
				this.#emitNode([1, 0], NODE_KIND_TEST, "blocked missing");
				return true;
			case "2":
				this.#emitNode([2, 0], NODE_KIND_TEST, "plain ready");
				return true;
			case "0.0":
			case "1.0":
			case "2.0":
