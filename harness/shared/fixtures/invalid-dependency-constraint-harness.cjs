...
class FakeHarness {
	...
	discover(nodeIndex) {
		switch (Array.isArray(nodeIndex) ? nodeIndex.join(".") : "<invalid>") {
			case "":
				this.#emitNode([0], NODE_KIND_SUITE, "constraint root");
				return true;
			case "0":
				this.#emitNode([0, 0], NODE_KIND_TEST, "invalid dependency target");
				this.#emitNode([0, 1], NODE_KIND_TEST, "plain ready child");
				return true;
			case "0.0":
			case "0.1":
				return false;
