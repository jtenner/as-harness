...
use std::cell::Cell;
use wasmtime::{AsContextMut, Caller, Engine, Extern, Instance, Linker, Memory, Module, Store, TypedFunc};
  ...
  fn instantiate(&self) -> Result<(Store<HostState>, Instance)> {
    ...
    linker
      .func_wrap(
        "as-harness",
        "write_event",
        |mut caller: Caller<'_, HostState>, kind: u32, payload_ptr: u32, payload_len: u32| -> AnyResult<()> {
    ...
    linker
      .func_wrap(
        "env",
        "abort",
        |mut caller: Caller<'_, HostState>, message_ptr: u32, file_name_ptr: u32, line: i32, column: i32| -> AnyResult<()> {
          let memory = get_memory_from_caller(&mut caller)?;
          Err(anyhow!(
            "abort: {} at {}:{}:{}",
