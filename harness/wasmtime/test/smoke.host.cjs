...
test("cli run executes uvu fixture and snapshot helpers through the wasmtime harness", () => {
	const tempDirectory = mkdtempSync(
		path.join(tmpdir(), "as-harness-wasmtime-snapshots-"),
	);
...
test("cli run reports mismatched and stale snapshots through the wasmtime harness", () => {
	const tempDirectory = mkdtempSync(
		path.join(tmpdir(), "as-harness-wasmtime-snapshot-mismatch-"),
	);
...
test("cli run rewrites snapshots in update mode through the wasmtime harness", () => {
	const tempDirectory = mkdtempSync(
		path.join(tmpdir(), "as-harness-wasmtime-snapshot-update-"),
	);
...
test('cli run executes the bundled "uvu" guest library through the wasmtime harness', () => {
	const tempDirectory = mkdtempSync(
		path.join(tmpdir(), "as-harness-wasmtime-uvu-"),
	);
