...
for `v0.3.0` work rather than a permanent frozen standard.

The current shipped contract does not include scheduler-step entrypoints.
For the current `v0.3.0` direction, those remain deferred while the host-owned
scheduler executes planned tests through the existing targeted `run()`
primitive with staged `NodeIndex` values.
