...
This is the host-side Wasm wire contract for `as-harness`, meant for people writing a custom host instead of reusing the shipped host packages.

Use this with [007-2026-03-17-host-runner-contract.md](./007-2026-03-17-host-runner-contract.md).
...
This document is the wire layer; the JavaScript runner surface and `start()` flow are in
...
The project intends this ABI to stay flat and language-agnostic. It is still
early and not yet versioned independently, so treat it as the current contract
for the current pre-`v1` release line rather than a permanent frozen standard.

The current shipped contract does not include scheduler-step entrypoints.
Those remain deferred while the host-owned scheduler executes planned tests
through the existing targeted `run()` primitive with staged `NodeIndex`
values.

The current CLI compile path intentionally keeps the guest export surface flat:
the generated wrapper re-exports only `allocateNodeIndexBuffer`, `discover()`,
`invoke()`, and `run()`, with `memory` and `__start` as the only additional
guest exports expected by hosts.
...
The guest may also rely on the normal AssemblyScript abort and trace imports
from module `env`:

- `abort(messagePtr, fileNamePtr, line, column)`
- `trace(messagePtr, valueCount, a0, a1, a2, a3, a4)`

The shipped `as-harness` CLI wrapper may rewrite bare `abort(...)` and
`trace(...)` calls in harness-backed source builds onto bundled internal debug
helpers before `asc` parses those files. That wrapper rewrite does not change
the Wasm ABI itself: the raw `env.abort(...)` and `env.trace(...)` imports
remain the underlying AssemblyScript import surface, and direct `asc` builds
...
[only: u8]
[expect_failure: u8]
[preferred_runner_mode: u8]
[preferred_failure_policy: u8]
[padding: 1 byte]
[dependency_count: u32]
[dependency_node_id_0: u32] ... [dependency_node_id_n: u32]
...
This is the structured host-side payload used by the wrapper-owned debug helper
path for rewritten `abort(...)` and `trace(...)` calls.
...
repeat crumb_count:
  ...
  [source_file: utf8 bytes]
  [source_line: u32]
  [source_column: u32]
[message_byte_length: u32]
[message: utf8 bytes]
[location_file_byte_length: u32]
[location_file: utf8 bytes]
[location_line: u32]
[location_column: u32]
```
...
- decoding `Debug` events when the wrapper-owned debug helper path emits them
...
Discovered declaration metadata is also the host-owned scheduling input:

- `sequenceMode` and `dependencyNodeIds` are binding constraints
- `preferredRunnerMode` and `preferredFailurePolicy` are host-readable hints
- the guest may declare those fields, but only the host decides how and when
...
- `Debug` is the host-facing form of wrapper-rewritten `abort(...)` and
  `trace(...)` calls that preserve crumb and location metadata
...
Scheduling metadata affects host planning:

- `sequenceMode = sequential` is a binding constraint on the declaring scope
- `dependencyNodeIds` are binding prerequisite edges resolved through the
  dependent node's ancestor chain
- `preferredRunnerMode = in-band` is an advisory hint that asks the host to
  keep the nearest hinted scope on the main-thread lane
- `preferredFailurePolicy = bail` is an advisory hint that asks the host to
  stop the remaining nearest hinted scope after the first unsatisfied
  execution
- `preferredFailurePolicy = continue` is an advisory hint that opts a scope
  out of an inherited `bail`
...
- should emit `nodeFound` callbacks for structurally visible nodes reached by
  that discovery call
- decoded `NodeFound` values must preserve `sequenceMode`,
  `dependencyNodeIds`, `preferredRunnerMode`, and `preferredFailurePolicy`
...
- honor binding declaration constraints from `sequenceMode` and
  `dependencyNodeIds`
- read `preferredRunnerMode` and `preferredFailurePolicy` as host-owned
  planning hints
- execute each branch
