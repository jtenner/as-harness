...
- a thin Jest-shaped adapter is available when the compile path includes `--lib jest`, including a small `expect(...)` surface
...
- fuller Jest compatibility beyond thin declarations, core hooks, and the small shared-assertion-backed `expect(...)` surface
