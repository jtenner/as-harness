...
function hasGlobMagic(pattern: string) {
  return /[*?[{!]/.test(pattern);
}
...
function splitGlobPattern(pattern: string, cwd: string) {
  ...
  const rootPrefix = lastSlashIndex === -1 ? "" : normalizedPattern.slice(0, lastSlashIndex + 1);
  const scanPattern = normalizedPattern.slice(rootPrefix.length) || "**/*";
  const scanCwd = isAbsolute(pattern) ? rootPrefix || "/" : resolve(cwd, rootPrefix || ".");
...
function shouldIgnoreEntry(absolutePath: string, cwd: string, ignorePatterns: string[]) {
  ...
  return ignorePatterns.some(
    (pattern) => matchesGlobPattern(relativePath, pattern) || matchesGlobPattern(absolutePosixPath, pattern),
  );
}
...
async function discoverEntryFiles(command: ParsedCommand, cwd: string) {
  ...
  return [...candidates]
    ...
    .sort((left, right) => displayPath(left, cwd).localeCompare(displayPath(right, cwd)));
}
...
async function runRunCommand(command: ParsedCommand, cwd: string) {
  ...
  console.error(`Resolved ${entries.length} entry file(s). Compilation execution is not implemented yet.`);
  process.exitCode = 1;
}
