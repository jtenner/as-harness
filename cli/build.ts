...
function resolveTargets(args: string[]) {
  ...
  const invalidTargets = args.filter((target) => !validTargets.has(target as Bun.Build.CompileTarget));

  if (invalidTargets.length > 0) {
    console.error(`Unknown compile target(s): ${invalidTargets.join(", ")}`);
    console.error("Run 'bun run build:list-targets' to see the supported target list.");
    process.exit(1);
  }
...
async function buildTarget(target: Bun.Build.CompileTarget, index: number, total: number) {
  ...
  const processHandle = Bun.spawn(
    [process.execPath, "build", "--compile", `--target=${target}`, `--outfile=${outfile}`, ENTRYPOINT],
    {
      stdout: "inherit",
      stderr: "inherit",
      stdin: "inherit",
